...
# ✅ Test with Ayyappa Swamy Stotras (4 pages)
TEST_CATEGORY = {
...
def get_category_pages(category_slug, category_name):
    """Get all pages for a specific category"""
    try:
        category_url = f"{BASE_CATEGORY_URL}{category_slug}/"
        print(f"Fetching category: {category_name} from {category_url}")
        
        resp = requests.get(category_url, timeout=15)
        resp.encoding = "utf-8"
        
        if resp.status_code != 200:
            print(f"Error: HTTP {resp.status_code} for {category_url}")
            return []
            
        soup = BeautifulSoup(resp.text, "html.parser")
        
        pages = []
        # Look for links to individual stotra pages
        ...
        for link in links:
            ...
            # Filter for English stotra pages
            if (href.endswith("-english.html") or href.endswith("-english/")) and text:
                # Convert relative URLs to absolute
                if href.startswith("/"):
                    href = f"https://www.vignanam.org{href}"
...
def process_category(category_slug, category_name):
    ...
    pages = get_category_pages(category_slug, category_name)
    ...
    print(f"\n{category_name} completed: {success_count}/{len(pages)} pages successful")
    return success_count

# ✅ Main execution
def main():
    print("Starting Test Scraper for Ayyappa Swamy Stotras...")
    print("=" * 60)
    
    total_success = 0
    
    # Process test category
    for category_slug, category_name in TEST_CATEGORY.items():
        try:
            success_count = process_category(category_slug, category_name)
    ...
    print(f"\n{'='*60}")
    print(f"TEST SCRAPING COMPLETED!")
    print(f"Total pages processed successfully: {total_success}")
    print(f"{'='*60}")
