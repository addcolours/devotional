      ...
      <div class="container">
         <div class="my-3">
            <ul class="list">
               ...
               <li><a href="<?php echo $base_url; ?>/en/ayyappa-stotras">Ayyappa Swamy Stotras</a></li>
               <li>{title}</li>
            </ul>
         </div>
      </div>
...
def scrape_stotram_page(url, title):
    ...
    try:
        ...
        # Remove unwanted tags - including aqtree3clickable and relatedCategories
        for tag in content_div.find_all(["table", "style", "script", "noscript", "ul", "p"]):
            ...
            if tag.name == "ul" and tag.get("class") and "aqtree3clickable" in tag.get("class"):
                ...
                tag.decompose()
            # Remove relatedCategories p elements
            elif tag.name == "p" and tag.get("class") and "relatedCategories" in tag.get("class"):
                print("  Removing relatedCategories p element")
                tag.decompose()
            else:
                tag.decompose()
...
def process_pages(pages):
    ...
    for i, page in enumerate(pages, 1):
        ...
        output_path = os.path.join(OUTPUT_DIR, f"{page['slug']}.php")
        if os.path.exists(output_path):
            print(f"  File already exists: {page['slug']}.php")
            continue
        ...
        if content:
            # Create PHP file
            created_path = create_php_file(page['title'], content, page['slug'])
...
# ✅ Main execution
def main():
    print("Starting Test Single Page Scraper for Ayyappa...")
    print("=" * 60)
